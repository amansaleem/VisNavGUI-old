function kalmansim(gain,QuailAccel_noise_mag,NinjaVision_noise_mag)
...
C = [1 0;0 1]; % [1 0; 0 1]; % measurement matrix: the expected measurement given the predicted state (likelihood)
...
u(1:round(Nsamples/8)) = 1;
u(round(Nsamples/8)+1:2*round(Nsamples/4)) = 0;
u(2*round(Nsamples/4)+1:3*round(Nsamples/4)) = -0.25;
u(3*round(Nsamples/4)+1:end) = -0.25;
u(1) = 10;
...
Ez = [NinjaVision_noise_mag^2 0 ; 0 QuailAccel_noise_mag^2];%*[1 0; 0 1];% Ez convert the measurement noise (stdv) into covariance matrix
...
Q_loc_meas(2,:) = Q_loc_meas(2,:)/gain;
