function [es, VRdata, VRdata_o, chans] = getVR2pdata(animal,iseries,iexp,irecexp, ...
    discreet_steps, flag_load, igroup, Nplane,SmthTimeWindow)
...
if run_rest
    ...
    es.spikeTrain = [];
    icell = 0;
    FgetSpikes = true;%false if you want the fluorescence
    for igroup_idx = 1:length(igroup)
        if exist([dDIRname filesep 'F_' animal '_' num2str(iseries) '_plane' num2str(igroup(igroup_idx)) '_proc.mat'],'file')
            ...
            win2p = 2*floor(0.5*(nanmean(diff(neuralFrameTimes((igroup_idx):Nplane:end)))/nanmean(diff(es.sampleTimes))))+1;
            tmax = numel(es.sampleTimes);
            for n = 1:numCells
                icell = icell + 1;
                labelType = 'good';
                ...
                if isfield(dat,'sp') && FgetSpikes
                    pks = findpeaks_chronux(es.spikeTrain(:,icell),1);%findpeaks from Chronux toolbox
%                     pks.height = log(es.spikeTrain(pks.loc,icell));
%                     pks.loc(pks.height < 1) = [];
%                     pks.height(pks.height < 1) = [];
%                     pks.loc1 = max(1,pks.loc - floor(win2p/2));
%                     pks.loc2 = min(tmax,pks.loc + floor(win2p/2));
%                     es.spikeTrain(:,icell) = 0*es.spikeTrain(:,icell);
%                     nspktime = numel(pks.loc);
%                     for tspk = 1:nspktime
%                         spk2pwin = pks.loc1(tspk):pks.loc2(tspk);
%                         es.spikeTrain(spk2pwin(randperm(numel(spk2pwin),round(pks.height(tspk)))),icell) = 1;
%                     end
                    es.spikeTrain(:,icell) = 0*es.spikeTrain(:,icell);
                    es.spikeTrain(pks.loc,icell) = 1;

                    %if wanna use lookup table, try binning the amplitude of
                    %the response in x bins (with x <10 first)
                    %something like that or normalized by any other relevant measure of response amplitude
                    %es.spikeTrain(:,icell) = es.spikeTrain(:,icell)/max(es.spikeTrain(:,icell))*10;
                else
...
%     else
       ...
       % display('Problem with append, saving without append!')
        save([dDIRname filesep fname '_contspikes'],'VRdata','VRdata_o','es','-v7.3');
   % end
