...
allMap.Fdiscarditer = false;

allMap = allMap.trainSpikeMap(V, Y, T, Xsmth_win, obj.FoptiSmooth);
...
%     goodCells = ((max(cellfields)-min(cellfields))./(max(cellfields)+min(cellfields))>= 0.5 & sum(Y,1)/(1/60*size(Y,1)) < 5);
    ...
    [Posterior, nonNormPosterior] = calcPosterior(obj.model.trained(iter).respModel(goodCells,:), Ytest(:,goodCells), Ttest, 1./obj.numBins); % Get the posterior estimates
