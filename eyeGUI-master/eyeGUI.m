...
handles.binfolder = 'C:\Users\experiment\Documents\DATA SSD\eyetracking';
...
if folder_name ~= 0
    ...
    if isempty(filename{1})
        ...
        handles.settings = [handles.files{1}(1:find(handles.files{1} == '.',1,'last')-1) '_settings.mat'];
...
for jf = 1:length(handles.files)
    handles.files = handles0.files(jf);
    handles.folders = handles0.folders(jf);
    handles.filename = handles0.files(jf);
    handles.settings = [handles0.files{jf}(1:find(handles0.files{jf} == '.',1,'last')-1) '_settings.mat'];
    handles = SaveROI(handles);
    if ~isfield(handles0,'multifiles')
        ik = 1;
    elseif isempty(handles0.multifiles)
        ik = 1;
    else
        ik = length(handles0.multifiles)+1;
    end
    handles0.multifiles{ik} = handles.settings;
    if strcmp(handles.settings(1:length(handles.filepath)),handles.filepath)
        foldname = handles.settings(length(handles.filepath)+1:end);
    else
        foldname = handles.settings;
    ...
    handles0.multifilelabel{ik} = foldname;
end
...
[folds,didchoose] = listdlg('PromptString','which folders (ctrl for multiple)',...
    'SelectionMode','multiple','ListSize',[240 160],'ListString',handles.multifiles);
for j = folds
