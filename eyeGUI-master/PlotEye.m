...
jl = 1;
frames = zeros(handles.vr.Height, handles.vr.Width, tsc, 'uint8');
for j = 1:tsc
    ...
    if indROI < 3
        ...
        if indROI == 1
            ...
            if isfield(params,'fr')
                hold all;
                [ysel,xsel] = find(params.fr > 20);
                scatter(xsel,ysel,10,'+','MarkerEdgeColor',[0.5 0.5 0.5]);
            end
            if params.isgood
                hold all;
