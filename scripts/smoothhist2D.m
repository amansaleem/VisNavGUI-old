function [F, ctrs1, ctrs2] = smoothhist2D(X,lambda,nbins,Fcircular1,Fcircular2,outliercutoff,plottype)
...
if nargin < 6 || isempty(outliercutoff), outliercutoff = .05; end
if nargin < 7, plottype = 'none'; end
...
F = special_smooth2D(K,[lambda(2)*repY/size(K,1) lambda(1)*repX/size(K,2)]);
% G = smooth1D(K,lambda);
% F = smooth1D(G',lambda)';
if Fcircular1
