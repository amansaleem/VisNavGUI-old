function kalmansim2(gain,QuailAccel_noise_mag,NinjaVision_noise_mag)
...
duration = 5;  %how long the Quail flies
...
Nstate = 1;
A = [1] ; % state transition matrix:  expected flight of the Quail (state prediction)
B = [dt]; %input control matrix:  expected effect of the input accceleration on the state.
C = [1]; % measurement matrix: the expected measurement given the predicted state (likelihood)
...
Q= [0]; %initized state--it has two components: [position; velocity] of the Quail
...
Ex = QuailAccel_noise_mag^2 * [dt^2]; % Ex convert the process noise (stdv) into covariance matrix
...
for t = 0 : dt: duration
    ...
    QuailAccel_noise = QuailAccel_noise_mag * randn;
...
% plot(0:dt:t, vel, '-b.',0:dt:t, acc, '-k.');
...
for t = 1:length(Q_loc)
    ...
    % Kalman Gain
    if (Q_loc_meas(t)>600 && Q_loc_meas(t)<700) || (Q_loc_meas(t)>1500 && Q_loc_meas(t)<1600)
        K = P*C'/(C*P*C'+Ez/100);
    else
        K = P*C'/(C*P*C'+Ez);
    end
...
figure(2);
tt = 0 : dt : duration;
plot(tt,Q_loc,'-r.',tt,Q_loc_meas/gain,'-k.', tt,Q_loc_estimate,'-g.',tt,Q_loc_estimate-Q_loc,'-m.',tt,tt*0,'k');
axis([0 10 -30 80])
axis tight
